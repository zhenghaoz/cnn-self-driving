...
def learn(env,
    ...
    for t in range(num_iterations):
        ...
        if model_initialized and np.random.uniform() > exploration.value(t):
            ...
            q = session.run(target_q, {obs_tp1_float: [obs_context]})[0]
            action = np.argmax(q)
        else:
            action = np.random.randint(0, num_actions)
        last_obs, reward, done, _ = env.step(action)
        # Max time step limitation
        step_this_ep += 1
        if step_this_ep >= ep_len:
            done = True
        total_reward += reward
        ...
        if t % log_steps == 0 and model_initialized:
            print("Timestep %d" % (t,))
            print("mean reward (10 episodes) %f" % np.mean(returns[-10:]))
            print("best mean reward (10 episodes) %f" % np.max(returns[-10:]))

import config

# Icons
ICON_START_VIDEO_RECORD = config.DIR_ASSETS + 'start_video_record.png'
ICON_STOP_VIDEO_RECORD = config.DIR_ASSETS + 'stop_video_record.png'
ICON_START_DATA_RECORD = config.DIR_ASSETS + 'start_data_record.png'
ICON_STOP_DATA_RECORD = config.DIR_ASSETS + 'stop_data_record.png'
ICON_START_TRAIN = config.DIR_ASSETS + 'start_training.png'
ICON_SELF_DRIVING_ON = config.DIR_ASSETS + 'self_driving_on.png'
ICON_SELF_DRIVING_OFF = config.DIR_ASSETS + 'self_driving_off.png'
ICON_SAVE = config.DIR_ASSETS + 'save.png'
ICON_OPEN = config.DIR_ASSETS + 'open.png'
ICON_GITHUB = config.DIR_ASSETS + 'github.png'
ICON_ICON = config.DIR_ASSETS + 'icon.png'
ICON_CAMERA = config.DIR_ASSETS + 'camera.png'

# Images
IMAGE_OFFLINE = config.DIR_ASSETS + 'offline.jpg'

# Strings
STRING_START_SELF_DRIVING = 'Start Self-Driving'
STRING_STOP_SELF_DRIVING = 'Stop Self-Driving'
STRING_START_VIDEO_RECORD = 'Start Video Record'
STRING_STOP_VIDEO_RECORD = 'Stop Video Record'
STRING_START_DATA_RECORD = 'Start Data Record'
STRING_STOP_DATA_RECORD = 'Stop Data Record'

STRING_USAGE = '''
W   -   Forward
S   -   Backward
A   -   Turn Left
D   -   Turn Right
Up  -   Self-driving Switch
'''
